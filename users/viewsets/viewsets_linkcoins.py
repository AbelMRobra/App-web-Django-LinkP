...
class LinkcoinsViewset(viewsets.GenericViewSet):
    ...
    def reporte_entrega(self, request, pk=None):
        quey_canje = EntregaMoneda.objects.filter(moneda__usuario_portador__identificacion=pk).annotate(cantidad=Count('mensaje')).order_by('-fecha')
        serializer = linkcoins_serializers.EntregasSerializer(quey_canje, many=True)
